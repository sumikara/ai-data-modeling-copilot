...
> Profiling decomposition note: currently implemented primarily in `src/profiling/profile_runner.py` with supporting modules (`csv_reader.py`, `column_profiler.py`, `key_detector.py`, `relationship_detector.py`). Future refactors may further separate responsibilities.
